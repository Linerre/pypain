...
def extract_timeline_blocks():
    with open(src_file, 'r', encoding='utf8') as f:
        raw = f.read()
        lines = [line for line in raw.split('\n\n') if len(line) > 1 ]
        return lines

def remove_timelines(lines, startbl, endbl):
    with open(trg_file, 'w') as nf:
        for line in lines:
            txts = line.split('\n')
...
if __name__ == '__main__':
    lines = extract_timeline_blocks()
    remove_timelines(lines, start_block, end_block)
